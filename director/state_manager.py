...
class GameStateManager:
    ...
    def get_narrative_context(self, window_seconds: int = 300) -> str:
        """Generates a text summary of the last 5 minutes for the AI."""
        import time
        cutoff = time.time() - window_seconds
        
        # 1. Current Status
        if not self.current_state:
            return "Game Status: Unknown/Offline"
            
        run_duration = self.current_state.get('runDuration', 0)
        players = self.current_state.get('players', [])
        
        summary = [f"--- GAME STATE (Run Time: {run_duration}s) ---"]
        summary.append(f"Alive Players: {len(players)}")
        
        # 2. Player Status Lines
        for p in players:
            summary.append(f"- {p['username']}: {p['health']:.1f}HP, Loc: {p['dimension']}")
            
        # 3. Recent Events (The Story)
        summary.append("\n--- RECENT EVENTS (Last 5 mins) ---")
        recent_events = [e for e in self.event_history if e.get('timestamp', 0)/1000 > cutoff]
        
        if not recent_events:
            summary.append("No significant events.")
        else:
            for e in recent_events[-15:]: # Limit to last 15 to save context
                etype = e.get('eventType')
                data = e.get('data', {})
                if etype == 'player_chat':
                    summary.append(f"[CHAT] {data.get('player')}: {data.get('message')}")
                elif etype == 'player_death_detailed':
                    summary.append(f"[DEATH] {data.get('player')} died to {data.get('cause')}")
                elif etype == 'player_damaged' and data.get('damage', 0) > 4:
                     summary.append(f"[BIG HIT] {data.get('player')} took {data['damage']} dmg")
                elif etype == 'item_pickup': # From InventoryPickupItemEvent
                     summary.append(f"[LOOT] {data.get('player')} found {data.get('item')}")
